...
int main(void)
{
	...
	RfConfig.channel = DEFAULT_JOIN_CHN; //DEFAULT_JOIN_CHN;//DEFAULT_BUSS_CHN//DEFAULT_JOIN_CHN
...
uint8 mac_addr[8];
void nwk_date_turn_to_mac_to_usart(uint8* data)
{
	uint8 tt = 0;
	if(*data != 0x16 && *data != 0x1a) return;
	if(*(data+1) !=0x41 || *(data+2) !=0xc8) return;
	if(*(data+16) !=0xe4 || *(data+17) !=0xe5) return;
	if(*(data+18) !=0xf0 && *(data+18) !=0xf1) return;
	for(tt = 0;tt<8;tt++) mac_addr[tt] = *(data + 15 - tt);

	USART2_Send(mac_addr, 8);
}
