...
const struct QR_VERSIONINFO QR_VersonInfo[] = {{0}, // Ver.0
										 { 1, 												// Ver.1 nVersionNo
										   26,   											//ncAllCodeWord
											 19,   16,   13,    9,			//ncDataCodeWord[4];	
										   0,   											//ncAlignPoint;	
											 0,   0,   0,   0,   0,   0,//nAlignPoint[6];	
										   1,  26,  19,								//struct RS_BLOCKINFO RS_BlockInfo1[4]; 
										   1,  26,  16,
										   1,  26,  13,
										   1,  26,   9,
										   0,   0,   0,								//RS_BLOCKINFO RS_BlockInfo2[4];
										   0,   0,   0,
										   0,   0,   0,
										   0,   0,   0}
										};
...
void display_2weima(void)
{
	uint8 two_data[63] = {0};
	int t, tt;
  BYTE my_byModuleData[MAX_MODULESIZE + 3][MAX_MODULESIZE] = {0};

  memcpy(&my_byModuleData[3][0], &m_byModuleData[0][0], MAX_MODULESIZE*MAX_MODULESIZE);
  
 for(t = 0;t<21;t++)
 {
	 for(tt = 0;tt<8;tt++)
   {
		 if(my_byModuleData[tt][t]) two_data[t] |= 0x01<<tt;
	 }
 }
 ...
 for(t = 0;t<21;t++)
 {
	 for(tt = 0;tt<8;tt++)
   {
		 if(my_byModuleData[tt + 8][t]) two_data[t+21] |= 0x01<<tt;
	 }
 }
 ...
 for(t = 0;t<21;t++)
 {
	 for(tt = 0;tt<8;tt++)
   {
		 if(my_byModuleData[tt + 16][t]) two_data[t + 42] |= 0x01<<tt;
	 }
 }
 
 memset(backgroud, 0, 1024);
 Graph_Change( 32, 64, 21, 24, two_data, backgroud, 0);		
